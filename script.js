...
// ✅ แสดงจำนวนสินค้าในตะกร้าเมื่อโหลดทุกหน้า
document.addEventListener("DOMContentLoaded", () => {
    const cartCount = document.getElementById("cart-count");
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    ...
    // Sync ข้อมูลข้ามแท็บเบราว์เซอร์
    window.addEventListener("storage", () => {
        cart = JSON.parse(localStorage.getItem("cart")) || [];
        updateCartCount();
    });
...
document.addEventListener("click", (e) => {
    if (e.target.classList.contains("add-to-cart")) {
        ...
        localStorage.setItem("cart", JSON.stringify(cart));
        renderCart();  // ✅ ตรงนี้จะรวม updateCartCount() ด้วย
    }
});
document.addEventListener("DOMContentLoaded", () => {
    const cartCount = document.getElementById("cart-count");
    ...
    updateCartCount();  // ✅ เรียกให้แสดงจำนวนสินค้าทันทีเมื่อเข้าแต่ละหน้า

    window.addEventListener("storage", updateCartCount); // Sync ข้าม tab
});
// ปิด Sidebar/Dropdown ก่อนจะเปลี่ยนหน้า
...
document.getElementById("loginBtn")?.addEventListener("click", async (e) => {
    ...
    try {
      const response = await fetch("login.html"); // โหลดเนื้อหา login แบบ dynamic
      const html = await response.text();
      loginContent.innerHTML = html;
  
      import('./script.js').then(module => {
        module.initLoginEvents(); // ✅ ผูกปุ่ม login/register หลังโหลดเนื้อหา
      });
    } catch (err) {
      loginContent.innerHTML = "<p>เกิดข้อผิดพลาดในการโหลดแบบฟอร์ม</p>";
    }
